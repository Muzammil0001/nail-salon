...
const OrderPaymentQrDialog: React.FC<QRPaymentProps> = ({ open, onClose, order, setPaymentSuccessCheck }) => {
    ...
    useEffect(() => {
        ...
        if (open && order?.services?.length > 0) {
          const createQR = async () => {
            try {
              timeout = setTimeout(() => {
                controller.abort();
                ToastErrorMessage('Request canceled, Try Again');
                onClose();
              }, 15000);
      
              const { data } = await axios.post(
                '/api/strip/createqrpaymentlink',
                {
                  amount: convertToSubcurrency(order?.total),
                  order,
                },
                {
              ...
              if (error.name === 'CanceledError' || axios.isCancel(error)) {
                console.warn('QR Code request canceled due to timeout (15s)');
                ToastErrorMessage('Request canceled, Try Again');
                onClose();
              } else {
                console.error('QR Code generation failed:', error);
                ToastErrorMessage('QR Code generation failed, Try Again');
              }
            } finally {
              clearTimeout(timeout);
              setLoading(false);
        ...
        return () => {
          clearInterval(interval);
          clearTimeout(timeout);
          controller.abort(); // Abort if component unmounts or deps change
        };
      }, [order]);
    ...
    useEffect(() => {
        ...
        if (open && orderId) {
            pollInterval = setInterval(async () => {
                try {
            setLoading(true)
                    const { data } = await axios.post("/api/orders/orderstatusverify", {
                        id: orderId,
                    });
                    if (data.verified) {
...
export default OrderPaymentQrDialog;
