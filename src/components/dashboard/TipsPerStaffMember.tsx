...
export const TipsPerStaffMember = ({
  session,
  keys,
  data,
  loading,
  ...
  const [salesPerStaffMember, setSalesPerStaffMember] = useState({
    series: [],
    options: {
  ...
  const fetchSlaesPerStaff = async () => {
    try {
      setLoading(true);
      const response = await axios.post(
        "/api/dashboard/fetchtipsperstaff",
        salesPerStaffFilters
      );
      setSalesPerStaffMember((prev) => ({
        ...prev,
  ...
  return (
    <Grid container spacing={1}>
      <Grid size={12}>
        <Typography variant="h5">{t("tips_per_staff_member", keys)}</Typography>
      </Grid>
      ...
      <Grid size={{ xs: 12, lg: 6 }}>
        <div className="text-center">
          <Typography variant="h5" fontWeight={900} title={salesPerStaffMember.total.toString()}>
            {t("total", keys)} : ${formatNumberWithUnit(salesPerStaffMember.total)}
          </Typography>
          <Chart
            options={salesPerStaffMember.options}
            series={salesPerStaffMember.series.map((series: any) => ({
              ...series,
