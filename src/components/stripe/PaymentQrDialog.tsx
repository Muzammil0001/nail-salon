...
const QRPayment: React.FC<QRPaymentProps> = ({ open, onClose, reservation, setPaymentSuccessCheck }) => {
  ...
  useEffect(() => {
    ...
    if (open && reservation?.reservation?.length > 0) {
      const createQR = async () => {
        try {
          ...
          const { data } = await axios.post(
            ...
            {
              amount: convertToSubcurrency(reservation?.final_price),
              reservation,
            },
            {
