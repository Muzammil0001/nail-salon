  ...
  try {
    ...
    if (session) {
      ...
      if (!adyenResponse?.id) {
        ...
        return res.json({ message: "adyen_response_is_required" });
      }
      let nextpayment;
      ...
      try {
        ...
        if (response?.data?.status === "completed") {
          const user = await prisma.user.update({
            where: {
              id: req.body.onboardingData.user.id,
            },
            data: {
          ...
          try {
            ...
            const address = `${
              req.body.user.street ? req.body.user.street + ", " : ""
            }${req.body.user.postcode ? req.body.user.postcode + " " : ""}${
              req.body.user.city ? req.body.user.city : ""
            } ${req.body.user.country ? req.body.user.country : ""}`;
            const invoice_number = 0;
            const pdfFilename = await generatePDF({
              merchant_name: `${req.body.user.firstname}  ${req.body.user
                .lastname || ""}`,
              merchant_address: address,
              merchant_pin: req.body.user.business_invoice
                ? req.body.user.vat_id
                : "",
              pdf_name: "",
          ...
          res.status(StatusCodes.BAD_REQUEST);
          return res.json({ message: "adyen_response_is_not_completed" });
        }
      } catch (error) {
        res.status(StatusCodes.BAD_REQUEST);
        return res.json({ message: "adyen_response_is_not_completed" });
