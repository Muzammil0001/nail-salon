...
const MyForm = () => {
  ...
  const formik = useFormik({
    initialValues: {
      ...
      location_phone: "",
      language_id: null,
      location_timezone: "Europe/Zagreb",
      ...
      if (action === "create") {
        try {
          ...
          if (response.status === 201) {
            ...
            setTimeout(() => {
              window.location.href = "/admin/locations-overview";
            }, 1500);
          }
        } catch (error) {
        ...
        try {
          setLoading(true);
          const response = await axios.post("/api/location/editlocation", {
            ...allValues,
            id: router.query.id,
          });
  ...
  const fetchLocation = async (id: string) => {
    try {
      const response = await axios.post("/api/location/getsinglelocation", {
        id,
      });
