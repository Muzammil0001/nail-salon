...
const Appointments = () => {
    ...
    onSubmit: async (values) => {
      try {
        ...
        if (router.query.id) {
          const payload = {
            ...values,
            reservation_id: router.query.id,
          }
          const response = await axios.post("/api/reservation/updatereservation", payload);
    ...
    if (
      ...
      (async () => {
        try {
          ...
          formik.setValues({ ...response.data.reservation, payment_method: "cash" });
  ...
  useEffect(() => {
    fetchCategories()
    fetchServices()
  }, [formik.values.location_id, router.query])

  useEffect(() => {
    formik.setFieldValue("location_id", session?.user?.selected_location_id);
  }, [session]);
  ...
  useEffect(()=>{
    if(formik.values.reservation?.length > 0 && router.query.action==="edit" && Object.keys(servicesByCategory || {}).length > 0)
      {updateReservationsAndTotal(formik.values.reservation || []);}
  }, [ servicesByCategory])

  const updateReservationsAndTotal = (updatedReservations: any[]) => {
    if (!updatedReservations || updatedReservations.length === 0) {
      formik.setFieldValue("reservation", []);
      ...
      {((session?.user?.roles?.includes("Owner") && session.user.navigation?.includes("/admin/appointments")) ||
        (session?.user?.roles?.includes("BackOfficeUser") &&
          checkAccess(
            ...
            "/admin/appointments",
            router.query.action === "create" ? "add" : "edit"
          ))) ? (<PageContainer
            topbar={
              <HorizontalStepper
    ...
    {formik.touched.location_id && formik.errors.location_id && (
      <FormHelperText error>{formik.errors.location_id}</FormHelperText>
                        ...
                        <Button
                          ...
                          disabled={!newCategory || !newService || formik.values.payment_method === "card"}
                        >
                          {t("add_service", keys)}
                        </Button>
                      </Box>
                        ...
                        <Card sx={{ p: 3, mt: 4 }}>
                          ...
                          {formik.values.total_price && (
                            <Box sx={{ display: "flex", justifyContent: "space-between", py: 1, textTransform: "capitalize" }}>
                              ...
                              <Typography variant="body2" fontWeight={700}>${formik.values.total_price}</Typography>
                            </Box>
                          )}
                        </Card>
                      </Grid>
